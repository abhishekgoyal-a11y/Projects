...
> **Codex note:** Use `$webresearch` instead of `/webresearch`. Also add `multi_agent = true` under `[features]` in `~/.codex/config.toml`.
...
Load pages, split into batches of 5, dispatch ALL subagents in one message. Each subagent reads its batch and writes a chunk file.

Subagent prompt template:
```
You are a webresearch extraction subagent. Read the pages and extract structured facts.
...
[{"url":"...","title":"...","facts":[{"claim":"...","confidence":"DIRECT|INFERRED|AMBIGUOUS","quote":null}],"entities":["..."],"claims":[{"claim":"...","stance":"positive|negative|neutral","confidence":"DIRECT"}],"input_tokens":0,"output_tokens":0}]
```

Each subagent writes its result:
```bash
$(cat research-out/.webresearch_python) -c "
import json; from pathlib import Path
result = SUBAGENT_JSON
...
print(f'Merged {len(chunks)} chunks → {len(all_ex)} extractions')
"
```
...
Show outputs location and paste Key Findings + Contradictions + Suggested Questions from RESEARCH_REPORT.md.

## Honesty Rules
- Never invent facts. Mark uncertain items AMBIGUOUS.
- Always show source count found vs fetched.
